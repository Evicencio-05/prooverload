...
Vercel compiles `/api` from TypeScript using `api/tsconfig.json` (and root `compilerOptions` as a fallback). It does not follow this repo’s Vite project references, so those function files use extensionless imports and `@types/node` instead of `.ts` specifiers.
